        ...
        foreach ($rels->Relationship as $rel) {
            switch ($rel["Type"]) {
                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    ...
                    $change="";
                    if(isset($xpath['Target']))
                    {
                        $change=$xpath['Target'];
                    }
                    
                    $xmlStrings = simplexml_load_string($this->securityScan($this->getFromZipArchive($zip, "$dir/$change")), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
...
//                                                echo 'Default', PHP_EOL;
    ...
    //                                    echo 'Value is ', $value, PHP_EOL;
                    ...
                    if ((!$this->readDataOnly) || (!empty($this->loadSheetsOnly))) {
                        ...
                        {
                            $change=$xmlWorkbook->bookViews->workbookView["activeTab"];
                        }
                        $activeTab = intval($change); // refers to old sheet index
